...
#linkTo(route="docVersion", class="btn btn-default", version=params.version, encode="attributes", text="<i class='fa fa-reply'></i> Return to #params.version# docs")#
	...
	<div id="#lcase(meta.name)#"
		...
		class="functiondefinition">
			...
			<h3 class="functitle">#meta.name & "()"# <span class="dropdown ">
		  <button id="dLabel1" type="button" class="btn btn-xs btn-default" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">

		    <span class="caret"></span>
		  </button>
		  <ul class="dropdown-menu" aria-labelledby="dLabel1">
		    <li>#linkTo(route="docFunction", encode="attributes", version=params.version, slug=meta.slug, format="html", text="<i class='fa fa-anchor'></i> Permalink")#</li>
		    <li>#linkTo(route="docFunction", encode="attributes", version=params.version, slug=meta.slug, format="json", text="<i class='fa fa-code'></i> JSON")#</li>
		  </ul>
		</span></h3>
			  <hr />
			<p>
			<cfif len(meta.tags.section)>
				<i class="fa fa-tag"></i> #meta.tags.section#
			</cfif>
			<cfif len(meta.tags.category)>
				<i class="fa fa-tag"></i> #meta.tags.category#
			</cfif>
		...
		<cfif hasRelatedFunctions>
			<h3>Related Functions</h3>
			<h4>#docFunction.tags.category#</h4>
			<ul>
			<cfloop from="1" to="#arraylen(relatedFunctions)#" index="func">
				<li>#linkTo(route="docFunction", version=params.version, slug=relatedFunctions[func]["slug"], text=relatedFunctions[func]["name"] & "()", format="html")#</li>
			</cfloop>
		</ul>
		</cfif>
	</div><!--/container-->
